...
import { Users, Copy, Share2, DollarSign, TrendingUp, Check, Loader, RefreshCw } from 'lucide-react';
...
export default function AffiliateProgram() {
  ...
  const generateReferralCode = () => {
    if (!user) return '';
    return `CH${user.id.slice(-6).toUpperCase()}`;
  };
  ...
  const fetchAffiliateStats = async () => {
    ...
    try {
      ...
      setAffiliateStats({
        ...
        thisMonthEarnings: Math.random() * 10,
        commissionRate: 10,
        pendingPayments: Math.random() * 5
      });
  ...
  const copyReferralCode = async () => {
    try {
      await navigator.clipboard.writeText(affiliateStats.referralCode);
      setCopied(true);
  ...
  if (!user) {
    return (
      <div className="text-center text-gray-400 py-8">
        ...
        <p>Please sign in to access the affiliate program</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      >
        <div className="text-center mb-6">
          ...
          <h4 className="text-xl font-bold text-white mb-2">Earn {affiliateStats.commissionRate}% Commission</h4>
          <p className="text-gray-400 text-sm">
            Invite friends and earn {affiliateStats.commissionRate}% of their net losses forever!
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
          ...
          <div className="text-xl font-bold text-white">{affiliateStats.totalReferrals}</div>
          <div className="text-xs text-gray-500">Active: {affiliateStats.activeReferrals}</div>
        </motion.div>
        ...
        >
          ...
          <div className="text-xs text-gray-500">
            This month: {formatCurrency(affiliateStats.thisMonthEarnings)}
          </div>
        </motion.div>
      </div>

      {/* Pending Payments */}
      {affiliateStats.pendingPayments > 0 && (
        <motion.div
        ...
        >
          ...
          <div className="text-white">
            {formatCurrency(affiliateStats.pendingPayments)} will be paid within 24 hours
          </div>
        </motion.div>
      )}
      ...
      >
        ...
        <div className="text-sm text-gray-300 space-y-2">
          ...
          <div className="flex items-start space-x-2">
            <div className="w-1.5 h-1.5 bg-purple-400 rounded-full mt-2 flex-shrink-0" />
            <span>When they sign up and play, you earn {affiliateStats.commissionRate}% of their net losses</span>
          </div>
          <div className="flex items-start space-x-2">
            <div className="w-1.5 h-1.5 bg-purple-400 rounded-full mt-2 flex-shrink-0" />
